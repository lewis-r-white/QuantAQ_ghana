# LOADS IN JUST THE DATE INFORMATION FROM SD CARD DATA AND INCORPORATES CLOUD COMPLETENESS TO GET A SENSE FOR DATA TRANSMISSION RATES
...
folders_to_process <- c(list.dirs(processed_feb2025, recursive = FALSE),
                        list.dirs(processed_oct2024, recursive = FALSE))
...
# write_rds(all_data, here("data", "missingness", "sd_time_data_20230815-20250129.rds"))


# READ IN DATA IF ALREADY GENERATED ----
all_data <- readRDS(here("data", "missingness", "sd_time_data_20230815-20250129.rds")) %>%
...
all_weeks <- data.frame(week = seq(as.Date("2023-08-13"), as.Date("2024-11-12"), by = "week"))
