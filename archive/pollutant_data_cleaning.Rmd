...
## If no SD card. Merge with empty. 
# full_sd_card <- tibble(
#   timestamp_iso = as.POSIXct(character()),  # <dttm>
#   co           = double(),
#   no           = double(),
#   no2          = double(),
#   o3           = double(),
...
if (is.null(new_ids) || length(new_ids) == 0) {
  stop("No monitor IDs found for '", w2$monitors, "'. Make sure data/new_fleet.csv exists and has a 'monitor' column.")
}
...
```{r}
original_fleet <- read_csv(here("data", "original_fleet.csv")) 
new_fleet <- read_csv(here("data", "new_fleet.csv")) 

## Here, I set implausibly low readings to NAs. Could update workflow so this is done above before saving corrected data. 

pm1_corrected <- read_rds(here("data", "pm", "final", "pm1_corrected_20240816_20250901.rds")) %>%
  mutate(pm1 = if_else(pm1 < 1, NA_real_, pm1),
         pm1_raw = if_else(pm1_raw < 1, NA_real_, pm1_raw))
...
pm10_corrected <- read_rds(here("data", "pm", "final", "pm10_corrected_20240816_20250901.rds")) %>%
  mutate(pm10 = if_else(pm10 < 1, NA_real_, pm10),
         pm10_raw = if_else(pm10_raw < 1, NA_real_, pm10_raw))
...
pm25_corrected <- read_rds(here("data", "pm", "final", "pm25_corrected_20240816_20250901.rds")) %>%
  mutate(pm25 = if_else(pm25 < 1, NA_real_, pm25),
         pm25_raw = if_else(pm25_raw < 1, NA_real_, pm25_raw))
