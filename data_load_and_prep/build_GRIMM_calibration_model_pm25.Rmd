...
modulair_full %>% 
  filter(mod_pm25 < 300) %>%
  ggplot(aes(x = mod_pm25)) +
  geom_histogram(bins = 20, color = "white") +
  theme_minimal() +
  labs(title = "Histogram of Modulair PM 2.5",
       x = "Modulair PM 2.5 (filtered below 300 to remove outliers)")

grimm_full %>%
  ggplot(aes(x = grimm_pm25)) +
  geom_histogram(bins = 20, color = "white") +
  theme_minimal() +
  labs(title = "Histogram of GRIMM PM 2.5",
       x = "GRIMM PM2.5",
       y = "Count") +
...
ba_raw <- mod_grimm_full %>%
  mutate(
    mean_pair = (mod_pm25 + grimm_pm25) / 2,
    diff_pair = mod_pm25 - grimm_pm25
  )
...
ggplot(ba_raw, aes(x = mean_pair, y = diff_pair)) +
  ...
  labs(
    ...
    x = "Mean of Modulair and GRIMM PM2.5",
...
calib_recipe <- recipe(
  grimm_pm25 ~ mod_pm25 + mod_temp + mod_rh + hour + season,
  data = train_day
) %>%
  step_mutate(
...
enet_preds <- predict(enet_fit_train, new_data = test_day) %>%
  bind_cols(test_day %>% select(grimm_pm25))
...
pearson_r2 <- cor(enet_preds$grimm_pm25, enet_preds$.pred, use = "complete.obs")^2

test_r2 <- 1 - sum((enet_preds$grimm_pm25 - enet_preds$.pred)^2) /
  sum((enet_preds$grimm_pm25 - mean(enet_preds$grimm_pm25))^2)

# calibration
cal_fit   <- lm(grimm_pm25 ~ .pred, data = enet_preds)
cal_int   <- coef(cal_fit)[1]
cal_slope <- coef(cal_fit)[2]

# shared axis limits
lims <- range(c(enet_preds$grimm_pm25, enet_preds$.pred), na.rm = TRUE)
...
ggplot(enet_preds, aes(x = .pred, y = grimm_pm25)) +
  geom_point(alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed", color = "black") +
  coord_equal(xlim = lims, ylim = lims) +
  labs(
    title = "Elastic Net: Predicted vs Observed PM2.5 (Day-blocked Test Set)",
    x = "Predicted PM2.5 (Elastic Net)",
    y = "Observed PM2.5 (GRIMM)",
...
ba_enet <- enet_preds %>%
  mutate(
    mean_pair = (.pred + grimm_pm25) / 2,
    diff_pair = .pred - grimm_pm25
  )
...
ggplot(ba_enet, aes(x = mean_pair, y = diff_pair)) +
  ...
  labs(
    ...
    x = "Mean of Predicted and Observed PM2.5",
    y = "Difference (Predicted - Observed)"
  )

## NORMALIZED RMSE
rmse_val <- rmse_vec(enet_preds$grimm_pm25, enet_preds$.pred)
mean_obs <- mean(enet_preds$grimm_pm25, na.rm = TRUE)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
xgb_recipe <- recipe(grimm_pm25 ~ mod_pm25 + mod_temp + mod_rh + hour + season, data = train_day) %>%
  step_mutate(
...
pearson_r2 <- cor(xgb_preds$grimm_pm25, xgb_preds$.pred, use = "complete.obs")^2

test_r2 <- 1 - sum((xgb_preds$grimm_pm25 - xgb_preds$.pred)^2) /
  sum((xgb_preds$grimm_pm25 - mean(xgb_preds$grimm_pm25))^2)


# calibration
cal_fit   <- lm(grimm_pm25 ~ .pred, data = xgb_preds)
cal_int   <- coef(cal_fit)[1]
cal_slope <- coef(cal_fit)[2]

# shared axis limits
lims <- range(c(xgb_preds$grimm_pm25, xgb_preds$.pred), na.rm = TRUE)
...
ggplot(xgb_preds, aes(x = .pred, y = grimm_pm25)) +
  geom_point(alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed", color = "black") +
  coord_equal(xlim = lims, ylim = lims) +
  labs(
    title = "XGBoost: Predicted vs Observed PM2.5 (Day-blocked Test Set)",
    x = "Predicted PM2.5 (XGBoost)",
    y = "Observed PM2.5 (GRIMM)",
    subtitle = "Red line = 1:1 identity; dashed line = regression fit",
    caption = caption_text
  ) +
...
## Normalized RMSE -----
rmse_val <- rmse_vec(xgb_preds$grimm_pm25, xgb_preds$.pred)
mean_obs <- mean(xgb_preds$grimm_pm25, na.rm = TRUE)
...
model_export <- mod_grimm_full %>%
  ...
  filter(!is.na(mod_pm25), !is.na(grimm_pm25))
...
readr::write_csv(
  ...
  here("data", "calibration", "mod_grimm_day_block_split_20260315.csv")
)
...
# 
# #set up cross validation 
# cv_folds <- vfold_cv(train, v = 5, strata = season)
```
...
#   coord_equal(xlim = lims, ylim = lims) +
#   labs(
#     title = "Elastic Net: Predicted vs Observed PM2.5 (Test Set)",
#     x = "Predicted PM2.5 (Elastic Net)",
#     y = "Observed PM2.5 (GRIMM)",
#     subtitle = "Red line = 1:1 identity; dashed line = regression fit",
#     caption = caption_text
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
### Evaluate on test set 
# xgb_preds <- collect_predictions(final_fit)
# 
# # metrics on test preds
# # metrics_df <- xgb_preds %>%
# #   yardstick::metrics(truth = grimm_pm25, estimate = .pred)
...
# cal_slope <- coef(cal_fit)[2]
# 
# # shared axis limits (use both obs + pred)
# lims <- range(c(xgb_preds$grimm_pm25, xgb_preds$.pred), na.rm = TRUE)
# 
# caption_text <- paste0(
#   "Test-set performance: RMSE = ", round(rmse_val, 2),
...
#   coord_equal(xlim = lims, ylim = lims) +
#   labs(
#     title = "XGBoost: Predicted vs Observed PM2.5 (Test Set)",
#     x = "Predicted PM2.5 (XGBoost)",
#     y = "Observed PM2.5 (GRIMM)",
#     subtitle = "Red line = 1:1 identity; dashed line = regression fit",
#     caption = caption_text
...
# # ----------------------------
# set.seed(123)
# split <- initial_split(model_export, prop = 0.8, strata = season)
# 
# random_train_ids <- training(split) %>% pull(row_id)
# random_test_ids  <- testing(split) %>% pull(row_id)
# 
# model_export_random_split <- model_export %>%
#   mutate(
#     split_random = case_when(
#       row_id %in% random_train_ids ~ "train",
#       row_id %in% random_test_ids  ~ "test"
#     )
#   )
# 
# # readr::write_csv(
# #   model_export_random_split,
# #   here("data", "calibration", "mod_grimm_random_split_20260301.csv")
# # )
```
