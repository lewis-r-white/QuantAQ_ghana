...
if (nrow(MOD_gas_sd_card) == 0 || is.null(MOD_gas_sd_card)) {
  message("⚠ No SD card gas data found — proceeding with cloud-only workflow")
...
This section explores the raw gas data during the colocation period to help identify which Modulair gas monitor behaves most consistently (“golden monitor”). It includes:
  - Daily gas time series by monitor
  - Pairwise correlations across monitors for CO, NO, NO₂, and O₃
...
For each gas: Corrected= (Raw−α)/β 

### Apply the correction equation 

If new monitors are added or a new golden monitor is chosen, run src/01_build_gas_calibration.R to regenerate the calibration table before running this workflow.
