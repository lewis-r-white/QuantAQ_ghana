...
daily_pm1_all_devices <- ghana_pm1 %>%
...
daily_cooktime_pm1_all_devices <- ghana_pm1 %>%
...
daily_cooktime_pm1_timeline <- ghana_pm1 %>%
  filter(hour >= 15 & hour < 18) %>%
  group_by(date) %>%
  summarize(mean_pm1 = mean(pm1, na.rm = TRUE)) %>%
  ggplot(aes(x = date, y = mean_pm1)) +
...
print(pm1_regression_results$plot)
print(pm1_regression_results$summary_table)
```
...
daily_pm10_all_devices <- ghana_pm10 %>%
  group_by(date, monitor) %>%
...
daily_cooktime_pm10_all_devices <- ghana_pm10 %>%
  filter(hour >= 15 & hour < 18) %>%
  group_by(monitor, date) %>%
...
daily_pm10_timeline <- ghana_pm10 %>%
  group_by(date) %>%
  summarize(mean_pm10 = mean(pm10, na.rm = TRUE)) %>%
...
daily_cooktime_pm10_timeline <- ghana_pm10 %>%
  filter(hour >= 15 & hour < 18) %>%
  group_by(date) %>%
  summarize(mean_pm10 = mean(pm10, na.rm = TRUE)) %>%
  ggplot(aes(x = date, y = mean_pm10)) +
...
print(pm10_regression_results$plot)
print(pm10_regression_results$summary_table)
```
