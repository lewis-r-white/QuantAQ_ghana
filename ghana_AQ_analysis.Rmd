...
location_data <- fread("/Users/lewiswhite/CHAP_columbia/QuantAQ/cloud/ghana_AQ_parent_full_20240925.csv", 
...
pm1_community_hourly <- results$pm1$summarized$hourly %>% filter(date >= as.Date("2023-09-26"))
pm1_community_daily <- results$pm1$summarized$daily %>% filter(date >= as.Date("2023-09-26"))
pm10_community_hourly <- results$pm10$summarized$hourly %>% filter(date >= as.Date("2023-09-26"))
pm10_community_daily <- results$pm10$summarized$daily %>% filter(date >= as.Date("2023-09-26"))
pm25_community_hourly <- results$pm25$summarized$hourly %>% filter(date >= as.Date("2023-09-26"))
pm25_community_daily <- results$pm25$summarized$daily %>% filter(date >= as.Date("2023-09-26"))


## Full Raw Merged Data
pm1_community_merged <- results$pm1$merged %>% filter(date >= as.Date("2023-09-26"))

pm25_full_merged <- results$pm25$merged
```

## Community Missingness (No SD card data during Colocation)

```{r sd card missingness, echo=FALSE, message=FALSE, warning=FALSE}
# calculate total missingness across all of the monitors and the entire time range of interest 
overall_missingness <- mean(is.na(pm1_community_merged$pm1)) * 100

# daily missingness  
daily_missingness <- pm1_community_merged %>% 
  group_by(date) %>% 
  summarise(missing_rate = mean(is.na(pm1)) * 100) %>%
...
# hourly missingingness
hourly_missingness <- pm1_community_merged %>% 
  group_by(hour) %>% 
  summarise(missing_rate = mean(is.na(pm1)) * 100)

#monitor missingness
monitor_missingness <- pm1_community_merged %>% 
  group_by(monitor) %>% 
  summarise(missing_rate = mean(is.na(pm1)) * 100) %>%
  mutate(monitor = fct_reorder(monitor, -missing_rate))
...
ggplot(monitor_missingness, aes(x = monitor, y = missing_rate)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        ...
        axis.text = element_text(size = 10))    # Increase axis labels size
...
### Spatial Correlation of Missing Data 
```{r sd card spatial missing data, echo=FALSE, message=FALSE, warning=FALSE}
# Convert to an sf object
missing_monitor_location <- st_as_sf(missing_monitor_location)
...
```{r sd card missingness heat map, fig.height=16, fig.width=9, echo=FALSE, message=FALSE, warning=FALSE}
## HEATMAP OF MISSINGNESS PER DAY FOR EACH MONITOR ----
pm1_community_merged_with_week <- pm1_community_merged %>%
  mutate(week = floor_date(date, "week")) # Extract the week


daily_na <- pm1_community_merged_with_week %>%
  group_by(monitor, date) %>%
  summarize(na_percent = sum(is.na(pm1)) / n() * 100) %>%
  ungroup() %>%
  mutate(week = floor_date(date, "week"),  # Extract the week again for plotting
