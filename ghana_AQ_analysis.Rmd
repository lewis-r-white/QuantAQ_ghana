...
pm1_daily %>%
  ...
  theme_minimal() +
  labs(y = "Daily mean PM 1",
       title = "Daily Average PM 1 for Each Monitor") 
...
ghana_pm1 %>%
  ...
  theme_minimal() +
  labs(y = "Mean PM 1",
       title = "Mean PM 1 Accross All Devices")
...
kable(moran_table, caption = "Moran I Test Results")
...
# Visualize spatial distribution of pollution levels. Plot with roads data
ggplot() +
  geom_sf(data = roads_filtered, color = "gray", size = 0.3) +  # Add roads data
  geom_sf(data = monitor_data, aes(geometry = geometry, color = mean_pm1), size = 5, alpha = 0.6) +  # Add monitor data
  scale_color_viridis_c() +
  labs(title = "Spatial Distribution of Mean PM1 Levels with Roads",
       color = "Mean PM1") +
  theme_minimal()
...
ggplot(monitor_sf_road_dist) +
  ...
  labs(title = "Relationship between Distance to Road and Mean PM1 Levels",
       ...
       y = "Average PM1 Reading") +
  theme_minimal()

# summary table of regression 
modelsummary(pm1_road_regression)
