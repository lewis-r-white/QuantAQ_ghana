...
pm10_colocation_daily <- results$pm1$daily %>% filter(date >= as.Date("2023-08-16") & date <= as.Date("2023-09-20"))
...
no2_colocation_daily %>%
  ggplot(aes(x = date, y = mean_no2)) +
  geom_line() +
  facet_wrap(~monitor)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
test <- co_colocation_hourly %>% filter(monitor == "MOD-00397") %>%
  mutate(monitor_00397 = mean_co) %>%
  select(monitor_00397, date, hour)

t <- co_colocation_hourly %>%
  left_join(test) %>%
